...
import { getResultRecord } from '../../../services/axios/apiCall/resultApi';
...
function User_Marks() {
    ...
    const handleClick = (attemptID) => {
        navigate(`/userdashboard/result/${attemptID}`)
    }

    const getRecord = async () => {
        if (userID) {
            const result = await getResultRecord(userID)
            setResults(result)
            setIsLoading(false)
        }
    }
                            ...
                            <div className="card">
                                <div className='card-body test'>
                                    ...
                                    <div className='table-action-bar d-flex align-items-center ps-3' style={{ height: "54px" }}>
                                        <h5 className='m-0'>Records ({results.length}) </h5>
                                    </div>
